...
export default function Dashboard() {
  ...
  // -----------------------------
  // LEADERBOARD DATA (GLOBAL, MATCHES /leaderboard)
  // -----------------------------
  useEffect(() => {
    const fetchLeaderboard = async () => {
      try {
        const usersSnap = await getDocs(collection(db, "users"));
        const contributorUsers = usersSnap.docs
        ...
        const leaderboardData = contributorUsers.map(user => {
                    ...
                    {tasks.length > 5 && (
                      <div className="px-6 pb-4 pt-2 border-t border-white/5 flex justify-end">
                        <motion.button
                          ...
                          type="button"
                          onClick={() => setIsTaskModalOpen(true)}
                          className="text-[10px] px-4 py-2 rounded-full bg-slate-900 border border-emerald-500/40 text-emerald-400 font-black tracking-[0.2em] uppercase"
                        >
                          Show_Full_Log
                        </motion.button>
                      </div>
            ...
            {isAdmin ? (
              ...
              <GlassCard className="p-8 sticky top-8 border-emerald-500/20" noHover>
                ...
                <div className="space-y-8">
                  <div className="bg-slate-900/60 rounded-2xl border border-emerald-500/20 p-5 flex items-center justify-between gap-4">
                    <div>
                      <p className="text-[9px] font-black text-slate-500 uppercase tracking-[0.3em]">You Are Contributing</p>
                      <p className="text-sm text-slate-300 font-medium mt-2">
                        Every answer you ship is moving the mission forward. Keep the streak alive.
                      </p>
                    </div>
                    <div className="flex flex-col items-end">
                      <span className="text-[10px] text-slate-500 uppercase tracking-[0.25em]">Sessions_Logged</span>
                      <span className="text-3xl font-black text-emerald-400 mt-1">
                        {submissions.filter(s => s.userId === userData.id).length}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="pt-4 border-t border-white/5 space-y-4">
                    <h4 className="text-[9px] font-black text-slate-600 tracking-[0.2em] uppercase">
                      Momentum_Badges
                    </h4>
                    {submissions.length === 0 && (
                      <p className="text-[10px] text-slate-500 font-mono">
                        First step: ship your first answer to start unlocking badges.
                      </p>
                    )}
                    {submissions.length >= 1 && (
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-2xl bg-emerald-500/10 flex items-center justify-center text-emerald-400 border border-emerald-500/30">
                          <CheckCircle2 size={18} />
                        </div>
                        <div>
                          <p className="text-xs text-white font-black uppercase">Signal_Online</p>
                          <p className="text-[10px] text-slate-500 font-mono tracking-tighter">
                            First contribution registered on the network.
                          </p>
                        </div>
                      </div>
                    )}
                    {submissions.length >= 10 && (
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-2xl bg-purple-500/10 flex items-center justify-center text-purple-400 border border-purple-500/30">
                          <Calendar size={18} />
                        </div>
                        <div>
                          <p className="text-xs text-white font-black uppercase">Flow_State</p>
                          <p className="text-[10px] text-slate-500 font-mono tracking-tighter">
                            10+ missions completed. Rhythm established.
                          </p>
                        </div>
                      </div>
                    )}
        ...
        {isTaskModalOpen && (
          ...
          >
            <motion.div
              ...
              className="relative w-full max-w-4xl mx-4 bg-slate-950 border border-indigo-500/40 rounded-3xl shadow-[0_30px_80px_rgba(15,23,42,0.9)] overflow-hidden"
            >
